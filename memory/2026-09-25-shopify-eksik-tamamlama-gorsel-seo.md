...
- **Beden sırası — gerçek sebep (canlı 011, 213 varyant)**: Shopify productSet seçenek DEĞER sırasını `productOptions.values`'tan değil varyantların İLK GÖRÜLDÜĞÜ sıradan kurar (ilk renkler Gri/Krem yalnız buçuklu → 35,5..40,5 önce). `productOptionsReorder` 28 varyantlı üründe çalışıyor, 100+ varyantlıda hatasız ama ETKİSİZ dönüyor. Kalıcı fix (cea2db9): urun_ac varyantları BEDEN→RENK sıralı gönderir; reorder sonucu yeniden okunup doğrulanır, uygulanmazsa sonuçta `uyari`. Canlı 011: productSet güncellemesi (aynı varyant id'leri, bedene göre sıralı) ile sıra düzeltildi, kapaklar düşmedi.
- **Kapak yalnız ilk 50 varyant tuzağı**: productSet yanıtında `variants(first: 50)`/`media(first: 50)` → 213 varyantlı 011'de yalnız 50 varyanta kapak atanmıştı (vitrin diğer renklerde ilk görseli gösteriyordu). 250'ye çıkarıldı (cea2db9). Canlı 011: galeri dosya adlarından (`011-<slug>-<renk>-<i>.jpg`) 18 rengin ilk görseli bulunup 163 varyanta kapak atandı → 213/213 kapaklı.
- **Sayfalama (4f8f0c4)**: `_tum_dugumler(pid, alan, secim)` product.variants/media'yı 250'lik sayfalarla tam çeker; urun_ac kapak ataması, eksik_tamamla (varyant+galeri, medya ekleme sonrası galeri), site_kapaksiz_renkler bunu kullanır. 250 sınırı da tuzak (18×13=234; 20 renkli model aşar).
