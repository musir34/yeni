...
- July 03, 2025. Fixed canvas and element responsive sizing. Canvas now uses direct mm units without pixel conversion. Added updateCanvasElements() function to automatically resize all canvas elements proportionally when canvas dimensions change. Elements (text, QR codes, barcodes, images) now scale appropriately with canvas size changes maintaining proper proportions
