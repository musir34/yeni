...
- July 01, 2025. Fixed QR code visibility issue in A4 print output. Implemented automatic position adjustment system that ensures QR codes always appear within label boundaries. QR codes positioned outside label area (due to coordinate scaling) are automatically relocated to safe positions with proper boundary detection. System now guarantees QR code visibility regardless of editor placement coordinates
