...
- July 03, 2025. Finalized Enhanced Product Label A4 system with complete Product Label compatibility. Implemented birebir A4_FIXED_CONFIG values (8mm left/right margins, 15mm top/bottom margins, 2mm column gap, 1mm row gap, 64.67×37.92mm calculated label dimensions). Replaced center positioning with margin-based straight layout matching Product Label coordinate system exactly. Enhanced Product Label A4 output now identical to original Product Label spacing and positioning while preserving drag-and-drop editor functionality
...
- July 03, 2025. Implemented direct mm coordinate system for Advanced Label Editor canvas. Removed pixel conversion (4px = 1mm) and implemented native mm units for canvas rendering. Canvas now displays in actual mm dimensions (canvas.style.width = width + 'mm'). Updated drag-and-drop positioning, element coordinates, and all measurement calculations to use mm units directly. Element positioning now uses 3.78px = 1mm conversion for display accuracy while storing coordinates in mm units internally
- July 03, 2025. Enhanced canvas sizing to match real mm values from right panel inputs. Canvas now displays at true 1:1 scale using 3.78px = 1mm conversion (96 DPI standard). Right panel mm values directly control canvas physical size for accurate design workflow. Added enhanced scale indicator showing actual dimensions and improved container padding for better workspace layout. Users can now design labels at actual printed size
