...
def cari_ekle():
    ...
    try:
        ...
        _log("CREATE", f"Finans cari hesap açıldı — {c.ad}")
...
def cari_alim(cari_id):
    try:
        ...
        _log("CREATE", f"Finans cari mal girişi — {h.cari.ad}: {h.tutar}₺", tutar=str(h.tutar))
        flash(f'✅ Mal girişi kaydedildi: {h.tutar:.2f} ₺. Borç bakiyesi {h.yeni_bakiye:.2f} ₺.', 'success')
    except FinansHata as e:
        flash(str(e), 'danger')
    return _geri(url_for('finans.cari_detay', cari_id=cari_id))
...
def cari_satis(cari_id):
    try:
        ...
        _log("CREATE", f"Finans cari satış — {h.cari.ad}: {h.tutar}₺", tutar=str(h.tutar))
        flash(f'✅ Satış kaydedildi: {h.tutar:.2f} ₺. Bakiye {h.yeni_bakiye:.2f} ₺.', 'success')
    except FinansHata as e:
        flash(str(e), 'danger')
    return _geri(url_for('finans.cari_detay', cari_id=cari_id))
...
def cari_odeme(cari_id):
    try:
        ...
        _log("CREATE", f"Finans cari ödeme — {h.cari.ad}: {tutar}₺ ({h.islem.hesap.ad})", tutar=str(tutar))
        flash(f'✅ Ödeme kaydedildi: {tutar:.2f} ₺ {h.islem.hesap.ad} hesabından düştü. '
              f'Borç bakiyesi {h.yeni_bakiye:.2f} ₺.', 'success')
    except FinansHata as e:
        flash(str(e), 'danger')
    return _geri(url_for('finans.cari_detay', cari_id=cari_id))
...
def cari_tahsilat(cari_id):
    try:
        ...
        _log("CREATE", f"Finans cari tahsilat — {h.cari.ad}: {tutar}₺", tutar=str(tutar))
        flash(f'✅ Tahsilat kaydedildi: {tutar:.2f} ₺ Beyazıt hesabına girdi. Bakiye {h.yeni_bakiye:.2f} ₺.', 'success')
    except FinansHata as e:
        flash(str(e), 'danger')
    return _geri(url_for('finans.cari_detay', cari_id=cari_id))
