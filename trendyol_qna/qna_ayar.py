"""
Q&A panel ayarları — AI taslakları için kalıcı genel talimat.

Saklama: motor_ayar.py'deki PlatformConfig 'ayar torbası' deseniyle aynı;
platform='qna_ayar' satırının extra_config JSON'unda tutulur. Migration GEREKMEZ.
Talimat boşsa taslak üretimi bugünkü davranışıyla birebir aynı kalır.
"""
import logging
...
def genel_talimat() -> str:
    ...
    ayar okunamadı diye taslak üretimi durmasın.
    """
    try:
        kayit = _kayit()
        ...
        logger.warning("[QNA-AYAR] genel talimat okunamadı", exc_info=True)
        db.session.rollback()
    return ""
