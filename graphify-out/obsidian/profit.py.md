---
source_file: "profit.py"
type: "code"
community: "E-posta Bildirimleri"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/E-posta_Bildirimleri
---

# profit.py

## Connections
- [[Decimal]] - `imports_from` [EXTRACTED]
- [[ModelDirekMaliyet]] - `imports` [EXTRACTED]
- [[ModelMaliyet]] - `imports` [EXTRACTED]
- [[OrderCancelled]] - `imports` [EXTRACTED]
- [[OrderCreated]] - `imports` [EXTRACTED]
- [[OrderDelivered]] - `imports` [EXTRACTED]
- [[OrderPicking]] - `imports` [EXTRACTED]
- [[OrderShipped]] - `imports` [EXTRACTED]
- [[Product]] - `imports` [EXTRACTED]
- [[ReturnOrder]] - `imports` [EXTRACTED]
- [[ReturnProduct]] - `imports` [EXTRACTED]
- [[__init__.py_4]] - `re_exports` [EXTRACTED]
- [[_fetch_usd_try()]] - `contains` [EXTRACTED]
- [[api_exchange_rate()]] - `contains` [EXTRACTED]
- [[d()]] - `contains` [EXTRACTED]
- [[format_number()]] - `contains` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[profit_report()]] - `contains` [EXTRACTED]
- [[save_missing_costs()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/E-posta_Bildirimleri