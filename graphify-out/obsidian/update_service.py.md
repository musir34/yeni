---
source_file: "update_service.py"
type: "code"
community: "Raf Yönetimi & Barkod Çakışması"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Raf_Ynetimi__Barkod_akmas
---

# update_service.py

## Connections
- [[CentralStock]] - `imports` [EXTRACTED]
- [[OrderCreated]] - `imports` [EXTRACTED]
- [[OrderHazirlaniyor]] - `imports` [EXTRACTED]
- [[OrderPicking]] - `imports` [EXTRACTED]
- [[Product]] - `imports` [EXTRACTED]
- [[RafUrun]] - `imports` [EXTRACTED]
- [[ShopifyConfig]] - `imports` [EXTRACTED]
- [[__init__.py_4]] - `re_exports` [EXTRACTED]
- [[_norm_bc()]] - `contains` [EXTRACTED]
- [[_norm_raf()_1]] - `contains` [EXTRACTED]
- [[_respond()]] - `contains` [EXTRACTED]
- [[_shopify_order_to_hazirla_format()]] - `imports` [EXTRACTED]
- [[archive.py]] - `imports_from` [EXTRACTED]
- [[confirm_packing()]] - `contains` [EXTRACTED]
- [[fetch_orders_from_api()]] - `contains` [EXTRACTED]
- [[find_order_across_tables()]] - `imports` [EXTRACTED]
- [[log_many()]] - `imports` [EXTRACTED]
- [[log_user_action()_1]] - `imports` [EXTRACTED]
- [[logger_config.py]] - `imports_from` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[normalize_barcode()]] - `imports` [EXTRACTED]
- [[order_service.py]] - `imports_from` [EXTRACTED]
- [[record_movement()]] - `imports` [EXTRACTED]
- [[shopify_config.py]] - `imports_from` [EXTRACTED]
- [[shopify_service.py]] - `imports_from` [EXTRACTED]
- [[stock_management.py]] - `imports_from` [EXTRACTED]
- [[sync_central_stock()]] - `imports` [EXTRACTED]
- [[test_bulk_pick.py]] - `imports_from` [EXTRACTED]
- [[trendyol_api.py]] - `imports_from` [EXTRACTED]
- [[update_order_status_to_picking()]] - `contains` [EXTRACTED]
- [[update_package_to_picking()]] - `contains` [EXTRACTED]
- [[user_logs.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Raf_Ynetimi__Barkod_akmas