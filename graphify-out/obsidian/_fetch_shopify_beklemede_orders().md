---
source_file: "siparis_hazirla.py"
type: "code"
community: "Stok Fix Testleri & Yardımcılar"
location: "L143"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Stok_Fix_Testleri__Yardmclar
---

# _fetch_shopify_beklemede_orders()

## Connections
- [[.is_configured()_2]] - `calls` [EXTRACTED]
- [[Shopify'dan beklemedeki siparişleri çek.     - Ödemesi onaylanmış (PAID) sipariş]] - `rationale_for` [EXTRACTED]
- [[_is_cod_order()]] - `calls` [EXTRACTED]
- [[get_home()]] - `calls` [EXTRACTED]
- [[get_queue_orders()]] - `calls` [EXTRACTED]
- [[siparis_hazirla.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Stok_Fix_Testleri__Yardmclar