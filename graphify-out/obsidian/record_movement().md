---
source_file: "stock_ledger.py"
type: "code"
community: "Raf Yönetimi & Barkod Çakışması"
location: "L121"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Raf_Ynetimi__Barkod_akmas
---

# record_movement()

## Connections
- [[MovementResult]] - `references` [EXTRACTED]
- [[StockMovement]] - `calls` [EXTRACTED]
- [[Tek bir stok hareketini deftere yazar (ve gerekiyorsa rafı mutasyona uğratır).]] - `rationale_for` [EXTRACTED]
- [[allocate_from_shelf_and_decrement()]] - `calls` [EXTRACTED]
- [[apply_lifecycle_effect()]] - `calls` [EXTRACTED]
- [[backfill_opening_balance.py]] - `imports` [EXTRACTED]
- [[confirm_packing()]] - `calls` [EXTRACTED]
- [[has_movement()]] - `calls` [EXTRACTED]
- [[main()_24]] - `calls` [EXTRACTED]
- [[normalize_barcode()]] - `calls` [EXTRACTED]
- [[pick_order_from_shelf()]] - `calls` [EXTRACTED]
- [[picking_service.py]] - `imports` [EXTRACTED]
- [[reconcile_phantom_ledger.py]] - `imports` [EXTRACTED]
- [[restore_stock_to_shelf()]] - `calls` [EXTRACTED]
- [[run_opening()]] - `calls` [EXTRACTED]
- [[run_reconcile()]] - `calls` [EXTRACTED]
- [[stock_ledger.py]] - `contains` [EXTRACTED]
- [[update_service.py]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Raf_Ynetimi__Barkod_akmas