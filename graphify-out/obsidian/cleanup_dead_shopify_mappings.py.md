---
source_file: "scripts/cleanup_dead_shopify_mappings.py"
type: "code"
community: "Ana Kasa Defteri"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Ana_Kasa_Defteri
---

# cleanup_dead_shopify_mappings.py

## Connections
- [[Shopify'da artık var olmayan ShopifyMapping kayıtlarını siler.  Önce diagnose_st]] - `rationale_for` [EXTRACTED]
- [[ShopifyMapping]] - `imports` [EXTRACTED]
- [[app.py]] - `imports_from` [EXTRACTED]
- [[main()_14]] - `contains` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[shopify_stock_service.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Ana_Kasa_Defteri