---
source_file: "claims_service.py"
type: "code"
community: "Maliyet Fişi & Tedarikçi"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Maliyet_Fii__Tedariki
---

# claims_service.py

## Connections
- [[Order]] - `imports` [EXTRACTED]
- [[Return]] - `imports` [EXTRACTED]
- [[__init__.py_4]] - `re_exports` [EXTRACTED]
- [[approve_claim()]] - `contains` [EXTRACTED]
- [[claims_list()]] - `contains` [EXTRACTED]
- [[fetch_claims_page()]] - `contains` [EXTRACTED]
- [[fetch_trendyol_claims_async()]] - `contains` [EXTRACTED]
- [[fetch_trendyol_claims_route()]] - `contains` [EXTRACTED]
- [[models.py]] - `imports_from` [EXTRACTED]
- [[process_all_claims()]] - `contains` [EXTRACTED]
- [[reject_claim()]] - `contains` [EXTRACTED]
- [[trendyol_api.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Maliyet_Fii__Tedariki