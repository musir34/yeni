---
source_file: "stock_alert_service.py"
type: "code"
community: "E-posta Bildirimleri"
location: "L46"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/E-posta_Bildirimleri
---

# alert_uncovered_orders()

## Connections
- [[ANLIK terfi edilemeyen (stoksuz) siparişlerden HENÜZ bildirilmemiş olanlar için]] - `rationale_for` [EXTRACTED]
- [[_order_to_info()]] - `calls` [EXTRACTED]
- [[build_stock_shortage_email()]] - `calls` [EXTRACTED]
- [[notify()]] - `calls` [EXTRACTED]
- [[promote_eligible_orders()]] - `calls` [EXTRACTED]
- [[promotion_service.py]] - `imports` [EXTRACTED]
- [[stock_alert_service.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/E-posta_Bildirimleri