    ...
    try:
        ...
        for element in elements:
            ...
            if element_type == 'title':
                ...
                # QR kod her etiket için kendi barkodunu kullanmalı
                if product_data and 'barcode' in product_data:
                    qr_data = product_data['barcode']  # Her etiketin kendi barkodu
                else:
                ...
                # Barkod her etiket için kendi barkodunu kullanmalı
                if product_data and 'barcode' in product_data:
                    barcode_data = product_data['barcode']  # Her etiketin kendi barkodu
                else:
...
def print_multiple_labels():
    ...
    try:
        ...
        A4_FIXED_CONFIG = {
            ...
            'LABELS_PER_PAGE': 21,
            'QR_SIZE_MM': 18,
            'LABEL_WIDTH_APPROX': (200 / 3),     # Sayfa genişliği / sütun sayısı (küçültüldü)
        ...
        # Etiketler arası boşluk sistemi - Sayfa sığdırma optimizasyonu
        margin_x = int((-10 / 25.4) * dpi) # -10mm sol kenar (daha çok sola çek)
        margin_y = int((20 / 25.4) * dpi)  # 20mm üst kenar boşluğu
        gap_x = int((2 / 25.4) * dpi)      # 2mm sütun arası boşluk (azaltıldı)
        gap_y = int((3 / 25.4) * dpi)      # 3mm satır arası boşluk
